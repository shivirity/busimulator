...
from consts import RATE_MAX_STOP, MAIN_LINE_STOP_TURN_THRESHOLD, MAIN_LINE_STOP_TURN_RATE_THRESHOLD, ONLY_MAIN_LINE_STOP_THRESHOLD
...
class RouteDecider:
    ...
    def decide_stop_action_multi(self, loc: str, bus_group: list, bus_info: dict, line: Line):
        """
        ...
        if side_id > 0.2:  # side line
            ...
            if len(dis_return_run_buses) > 0:
                decide_turn = {1: [bus for bus in bus_group if bus_info[bus].to_turn == 1],  # 正在等待的车中确定转向的
                               2: [bus for bus in bus_group if bus_info[bus].to_turn == 2]}  # 正在等待的车中确定转向的
                        ...
                        if side_1_down > 0.2 and side_2_down < 0.2:  # #1
                            ...
                            if side_1_down != side_2_down:
                                ...
                                turn_direc = 1 if sum([len(station['pool'])
                                                  ...
                                                  sum([len(station['pool'])
                                                       for station in
                                                       line.side_line[f'{main_id}#2'].side_stations.values()]) else 2
                            ...
                            if len(side_1_bus_num) + len(decide_turn[1]) > 0.2 and \
                                ...
                                if sum([len(station['pool'])
                                        for station in line.side_line[f'{main_id}#2'].side_stations.values()]) > 0.2:
                                    # 有人在#2等待
                                    dec_dict[bus] = {'stop': False, 'turn': 2, 'can_return_stop': True}
                                    decide_turn[2].append(bus)
                                    dec_num += 1
                                ...
                                if sum([len(station['pool'])
                                        for station in line.side_line[f'{main_id}#1'].side_stations.values()]) > 0.2:
                                    # 有人在#1等待
                                    dec_dict[bus] = {'stop': False, 'turn': 1, 'can_return_stop': True}
                                    decide_turn[1].append(bus)
                                    dec_num += 1
                                ...
                                sum_1_up = sum([len(station['pool'])
                                                for station in line.side_line[f'{main_id}#1'].side_stations.values()])
                                sum_2_up = sum([len(station['pool'])
                                                for station in line.side_line[f'{main_id}#2'].side_stations.values()])
            ...
            assert dec_num + len(stop_buses) == len(bus_group), f'{loc}, {dec_num}, {len(bus_group)}, {len(stop_buses)}'
